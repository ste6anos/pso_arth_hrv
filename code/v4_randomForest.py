...
def create_html_report(results, config, output_file='random_forest_report_dual_state_90m.html'):
    """Generate professional HTML report for Random Forest results with side-by-side comparison."""
...
<head>
    ...
    <title>Random Forest - Dual State Analysis</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
        ...
        .importance-bar-fill {{
            height: 100%;
            background: linear-gradient(90deg, #0366d6, #0366d6);
...
<body>
    <div class="container">
        <div class="header">
            <h1>Random Forest Analysis - Dual State Comparison</h1>
...
"""
    ...
    html += """
        ...
        <div class="footer">
            <p>Random Forest Regression | Dual State HRV Metrics Analysis</p>
        </div>
    </div>
</body>
</html>
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("RANDOM FOREST - DUAL STATE ANALYSIS")
    print("="*60)
    ...
    create_html_report(results, config, output_file='random_forest_report_dual_state_90m.html')
    ...
    for target, state_results in results.items():
        ...
        if 'low_hr' in state_results:
            summary_data.append({
                ...
                'Top_Feature': state_results['low_hr']['feature_importances'].iloc[0]['feature'],
        ...
        if 'high_hr' in state_results:
            summary_data.append({
                ...
                'Top_Feature': state_results['high_hr']['feature_importances'].iloc[0]['feature'],
                'Top_Feature_Importance': state_results['high_hr']['feature_importances'].iloc[0]['importance']
            })

    summary_df = pd.DataFrame(summary_data)
    summary_df.to_csv('random_forest_summary_dual_state.csv', index=False)
    print("\nSummary CSV saved to 'random_forest_summary_dual_state.csv'")
    print("\nDual-state Random Forest analysis complete!")
...
# Defining the window check parameters
window_offset = 100
window_size = 3

# Create output directories
output_dir_aw = Path(r"patient_metrics_windowduration300m/aw")
output_dir_sl = Path(r"patient_metrics_windowduration300m/sl")
...
# Defining the patients that will be tested so that there is not memory alloc errors. By using the following line, I bypass the limitation for excluding p2070001 and 2010001. However, I handle this issue when calc the final metrics on v3_dfploting. 
patients = [f"p{i}" for i in range(4010032, 4010060)]
...
for pnt in tqdm(patients, desc="Processing patients", leave=True):
    ...
    if not sl_output_path.exists():
        try:
            ...
            pnt_sl_metrics = FileUtils.group_and_apply_sliding_window_calculations(pnt_sl_df, window_offset, window_size, window_check_threshold=0.45)

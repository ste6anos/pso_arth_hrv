...
def create_html_report(results, config, output_file='symbolic_regression_reverse_dual_state_90m.html'):
    """Generate professional HTML report for reverse symbolic regression results with side-by-side comparison."""
...
<head>
    ...
    <title>Reverse Symbolic Regression - Dual State Analysis</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>Reverse Symbolic Regression Analysis - Dual State Comparison</h1>
            <p>Predicting HRV Metrics from Clinical Data: Low vs High HR Activity</p>
...
"""
    ...
    # Add individual HRV metric sections with side-by-side comparison
    for hrv_metric, state_results in results.items():
...
"""
    ...
    html += """
        ...
        <div class="footer">
            <p>Reverse Symbolic Regression with PySR | HRV Metrics Prediction from Clinical Data</p>
        </div>
    </div>
</body>
</html>
...
if __name__ == "__main__":
    ...
    APPLY_CAPPING = True       # Set to True to enable outlier capping (Z=3)
    VIF_THRESHOLD = 5.0        # Collinearity threshold (5.0 = moderate, 10.0 = high)

    print("="*60)
    print("REVERSE SYMBOLIC REGRESSION - DUAL STATE ANALYSIS")
    print("="*60)
    print("Predicting: HRV Metrics")
    print("From: Clinical Data")
    print("="*60)
    ...
    # Load HRV data for both states
    hrv_aw = pd.read_csv("window90m_hrv_mean_measurements_aw.csv", index_col=0)
    hrv_sl = pd.read_csv("window90m_hrv_mean_measurements_sl.csv", index_col=0)

    # Define clinical features (predictors) and HRV metrics (targets)
    clinical_features = ["DOC_PAT_BMI", "DAPSA_Score", "PASDAS", "PSAID_Final_Score", "CRP_mg_dL"]

    # HRV metrics to predict
    hrv_metrics = [
    ...
    # Prepare clinical data with patientid
    X_clinical = clinical_data_df[["patientid"] + clinical_features]

    results = {}

    # Train models for each HRV metric - DUAL STATE PROCESSING
    for hrv_metric in hrv_metrics:
        print(f"\n{'='*60}")
        print(f"TARGET: {hrv_metric}")
        print(f"{'='*60}")
        ...
        # Process both states
        for state_name, hrv_df in [('low_hr', hrv_sl), ('high_hr', hrv_aw)]:
            state_label = 'Low HR Activity' if state_name == 'low_hr' else 'High HR Activity'

            print(f"\n--- Processing {state_label} ---")

            # Prepare HRV data for this metric
            y_hrv = hrv_df[['patientid', hrv_metric]]

            # Merge clinical features with HRV target
            df = pd.merge(X_clinical, y_hrv, on="patientid")
            df = df.dropna(subset=[hrv_metric])
            df = df.dropna(subset=clinical_features)  # Drop rows with missing clinical data
            df = df.drop(["patientid"], axis=1)

            # Separate features (clinical) and target (HRV metric)
            X_raw = df[clinical_features]
            y_raw = df[hrv_metric]
            ...
            print(f"\n{'='*60}")
            print(f"TRAINING MODEL FOR {hrv_metric} - {state_label}")
            ...
            fig.suptitle(f'{hrv_metric} - {state_label} - Predictions vs Actual', fontsize=16, fontweight='bold')
            plt.tight_layout()
            ...
            print(f"\n{'='*60}")
            print(f"COMPLETED {hrv_metric} - {state_label}")
            print(f"{'='*60}")
    ...
    create_html_report(results, config, output_file='symbolic_regression_reverse_dual_state_90m.html')
    ...
    summary_df.to_csv('symbolic_regression_reverse_summary_dual_state.csv', index=False)
    print("\nSummary CSV saved to 'symbolic_regression_reverse_summary_dual_state.csv'")
    print("\nReverse dual-state analysis complete!")
